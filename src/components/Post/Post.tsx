...
import './Post.scss';
import { PostProps } from "../../types/componentsTypes";
...
const Post:FC<PostProps> = ({post}) => {
    ...
    function navigateTo (e:React.MouseEvent<HTMLDivElement>) {
        navigate('/posts-web/user/' + post.userId)
    };

    return(
        <div className="post">

            <Toast bg={'Light'.toLowerCase()} className="post__body">
                <Toast.Header closeButton={false} 
                                className="post__head"
                                onClick={navigateTo}>
                    <svg className="post__svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <strong className="me-auto post__title">{post.title}</strong>
                </Toast.Header>
                <Toast.Body>{post.title}</Toast.Body>
            </Toast>
